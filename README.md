...
формулой $L=k(V^2-V_{0}^2)$, где $V_{0}$ - скорость на которой колёса начинают отрываться от
земли,*k* - постоянное число для этого трамплина. Причем неважно где измерять скорость -
просто изменится $V_{0}$. Аэродинамикой можно либо пренебречь, либо добавить поправку из
графика выше, *k* можно либо вычислить на небольшом прыжке, либо взять из таблице ниже,
там я его рассчитал **для скорости в км/ч** и дальности в метрах.  
